# Graph Report - backflow-conflict-issue-guidance  (2026-07-28)

## Corpus Check
- 125 files · ~185,045 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1427 nodes · 3851 edges · 68 communities (50 shown, 18 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 398 edges (avg confidence: 0.8)
...
1. `Context` - 154 edges
2. `testGraph()` - 71 edges
3. `gitHarness()` - 71 edges
4. `Plan` - 70 edges
5. `checkout()` - 61 edges
6. `writeJSON()` - 56 edges
7. `newProvider()` - 51 edges
8. `newRepo()` - 41 edges
9. `Provider` - 40 edges
...
Cohesion: 0.10
Nodes (105): Plan, BackflowBranchName(), assertExclusionReason(), checkout(), commitOn(), conflictHarness(), containsSHA(), countBackflowOutcome() (+97 more)

### Community 1 - "Provider"
Cohesion: 0.16
Nodes (8): Provider, repoSettings, escapeRefPath(), Client, Mutex, Once, issueNumber(), nextLink()

### Community 2 - "github_test.go"
Cohesion: 0.07
Nodes (88): ghFake, ghFakeIssue, ghFakePull, issueSpec, prSpec, ghNum(), HandlerFunc, Mutex (+80 more)

### Community 3 - "Coordinator"
Cohesion: 0.08
Nodes (42): Action, BackflowExclusion, BackflowExclusionReason, BranchState, Commit, EdgeObservation, EdgeState, EdgeSummary (+34 more)
...
Cohesion: 0.11
Nodes (11): fakeForge, BranchPush, ConflictArtifact, ConflictArtifactID, ConflictArtifactSpec, MergeMethods, Reason, RequestID (+3 more)
...
Cohesion: 0.15
Nodes (28): ActionType, actionVerb(), edgeSummaryText(), exclusionCounts(), Commit, Writer, mdCell(), RenderJSON() (+20 more)
...
Cohesion: 0.06
Nodes (39): Live Merge-Method Fence, Merge-Commit Backflow Strategy, Skip-in-Range Fence, Git 2.45 Runtime Contract, Git Runner Shell-Out, Conflict Issue Marker-and-Label Identity, Durable Backflow Conflict Artifact, Lock-Free Conflict Issue Convergence (+31 more)
...
### Community 21 - ".send"
Cohesion: 0.31
Nodes (8): Duration, Header, Response, parseAPIError(), retryableStatus(), retryDelay(), serverBackoff(), sleepCtx()

### Community 22 - "ChangeRequest"
Cohesion: 0.16
Nodes (11): Agent Safety Rules (do not violate), Engine Purity Rules, Managed Change Requests, Layering Rule: entrypoint to engine to git/forge, depguard-enforced, ChangeRequest, RequestType, CreateRequest, RequestFilter (+3 more)
...
### Community 37 - "Forge"
Cohesion: 0.46
Nodes (14): Forge, assertAscending(), containsID(), T, mustCreate(), mustCreateArtifact(), mustList(), mustListArtifacts() (+6 more)
...
### Community 65 - "Parse"
Cohesion: 0.05
Nodes (59): adoPull, adoPullList, forkRef, gitRef, policyConfiguration, policyList, policyScope, policySettings (+51 more)

### Community 66 - "BuildPlan"
Cohesion: 0.07
Nodes (63): BackflowPolicy, Branch, Expectations, Promotion, FromConfig(), Branch, Graph, Expectations (+55 more)
...
Cohesion: 0.13
Nodes (19): apiError, apiError, errNoResponse, ghIssue, ghLabel, ghPull, ghRef, ghRepo (+11 more)
...
- **Why does `Context` connect `Context` to `Parse`, `Provider`, `tmpl.go`, `loadGraph`, `ParseRemoteURL`, `Coordinator`, `fakeForge`, `github.go`, `plan_reconcile_test.go`, `Provider`, `NewLogger`, `.send`, `ChangeRequest`?**
...
- **Why does `newRepo()` connect `newRepo` to `plan_reconcile_test.go`, `Context`?**
...
- **Should `reconcile_test.go` be split into smaller, more focused modules?**
  _Cohesion score 0.09778470058843891 - nodes in this community are weakly interconnected._