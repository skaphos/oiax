...
- Analysis remediation: Configuration revisions are ordered by verified Git ancestry, not worker start time or CAS arrival order. During ordinary reconciliation, an older or unorderable revision cannot replace newer durable notification policy. If the accepted commit is confirmed lost after a history rewrite, the explicit audited recovery in [ADR 0019](../../docs/adr/0019-audited-notification-revision-recovery.md) may accept the pinned policy without claiming ancestry; automated runs cannot take that path. Latency acceptance is measured at the bounded normal load defined under SC-001, separately from backlog recovery.
...
1. **Given** one transiently unavailable destination and one available destination subscribed to the same event, **When** delivery is attempted, **Then** the available destination receives the event and the unavailable destination is reported separately as retryable.
...
4. **Given** one or more notification deliveries fail, **When** reconciliation completes, **Then** Oiax returns the same exit status that the underlying reconciliation result would have produced without notifications and reports whether each failure remains retryable or is durably skipped.
...
- An enabled destination's named credential environment variable is absent during reconciliation: Oiax does not issue an HTTP request, reports a safe actionable diagnostic, keeps the delivery retryable until a deterministic receipt is durably recorded at the terminal threshold, and preserves the underlying reconciliation result and exit status.
...
- A runtime render fails before a message is saved: the delivery remains pending without consuming a claimed attempt; required identity is never truncated to make a transport envelope fit.
- A payload remains too large after optional commit summaries are removed: its first persisted failure receipt records `skipped/payload-too-large`. Any other deterministic failure receipt recorded at or after 24 total claimed attempts records `skipped/abandoned`; transient network, service, rate-limit and cancellation outcomes remain retryable.
- A result receipt cannot be persisted: a nonaccepted attempt reports `delivery-receipt-not-persisted` when no higher-priority state, storage or cancellation diagnostic applies, with its safe underlying outcome and status retained when present, but remains recoverable. No terminal-attempt guarantee applies without durable receipt persistence, attempt IDs have no universal count bound, and v1 performs no compaction.
- A proven older attempt reports acceptance after a skipped outcome: the late accepted receipt may establish durable delivery and wins over the earlier skipped state.
...
- **FR-014**: Notification delivery failure MUST NOT roll back, block, misstate, or change the existing exit status of the underlying managed-request reconciliation result. The failure MUST remain visible; retryable records MUST be eligible on a later reconciliation, while a durably skipped terminal record MUST NOT be automatically resent.
- **FR-015**: Delivery outcomes MUST distinguish at least successful, failed, retryable, and intentionally skipped events and MUST identify the affected destination by its non-secret configured name. A persisted `payload-too-large` receipt MUST skip on its first result; another deterministic outcome persisted at or after 24 total claimed attempts MUST skip as abandoned. Transient network, service, rate-limit and cancellation outcomes MUST remain retryable. Failed receipt persistence MUST NOT be presented as terminal state.
...
- **FR-026**: Templates MUST be declarative, validated before use, and unable to access runtime secrets or execute commands. Required event/request identification MUST remain available in a standard message footer or structured fields even when a custom template omits it, and MUST NOT be truncated to satisfy a transport cap. Rendering failures before the per-destination message is saved MUST leave the delivery pending without claiming an attempt, preserve core reconciliation results, and remain safely diagnosable.
